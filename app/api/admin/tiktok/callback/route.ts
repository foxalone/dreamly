...
import { completeOAuthCallback } from "@/app/api/admin/tiktok/_lib";
...
function redirectWith(request: Request, status: "connected" | "error", detail = "") {
  ...
  if (detail) url.searchParams.set("tiktok_error", detail.slice(0, 180));
...
export async function GET(request: Request) {
  try {
    ...
    console.error("[admin/tiktok/callback]", message);
