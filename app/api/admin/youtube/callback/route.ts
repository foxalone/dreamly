...
import { completeYouTubeOAuthCallback } from "@/app/api/admin/youtube/_lib";
...
function redirectWith(request: Request, status: "connected" | "error", detail = "") {
  ...
  if (detail) url.searchParams.set("youtube_error", detail.slice(0, 180));
...
export async function GET(request: Request) {
  try {
    ...
    await completeYouTubeOAuthCallback(code, state);
    return redirectWith(request, "connected");
  } catch (error) {
    const message = error instanceof Error ? error.message : "OAuth failed";
    console.error("[admin/youtube/callback]", message);
