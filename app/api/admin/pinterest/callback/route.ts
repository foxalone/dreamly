...
import { completePinterestOAuthCallback } from "@/app/api/admin/pinterest/_lib";
...
function redirectWith(request: Request, status: "connected" | "error", detail = "") {
  ...
  if (detail) url.searchParams.set("pinterest_error", detail.slice(0, 180));
...
export async function GET(request: Request) {
  try {
    ...
    await completePinterestOAuthCallback(code, state);
    return redirectWith(request, "connected");
  } catch (error) {
    const message = error instanceof Error ? error.message : "OAuth failed";
    console.error("[admin/pinterest/callback]", message);
