...
import { getFaithCopy, type FaithSlug } from "@/lib/i18n/hubs";
import { getLocalizedParents } from "@/lib/i18n/localize-dictionary";
import { getMessages } from "@/lib/i18n/messages";
import { absoluteLocaleUrl } from "@/lib/i18n/path";
import LocaleLink from "@/lib/i18n/LocaleLink";

export default function FaithHub({
  slug,
  ...
  const config = getFaithCopy(locale, slug);
  const parents = getLocalizedParents(locale);
  const pageUrl = absoluteLocaleUrl(`/dreams/${slug}`, locale);
  ...
  const collectionJsonLd = {
    ...
    mainEntity: {
      "@type": "ItemList",
      numberOfItems: parents.length,
      itemListElement: parents.map((entry, index) => ({
        "@type": "ListItem",
        position: index + 1,
        name: `${entry.title} — ${config.perspectiveLabel}`,
        url: `${absoluteLocaleUrl(`/dreams/${entry.canonicalSlug}`, locale)}#${slug}`,
      })),
    },
  };
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: config.heading, item: pageUrl },
    ],
  };
  ...
  return (
    <main>
      ...
      <div className="mx-auto max-w-5xl px-5 pb-12 pt-8 sm:px-8 sm:pt-12">
        ...
        <section className="mt-10" aria-label={config.perspectiveLabel}>
          <h2 className="text-2xl font-semibold tracking-tight">{t.chrome.browseBySymbol}</h2>
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {parents.map((entry) => (
              <LocaleLink
                key={entry.slug}
                href={`/dreams/${entry.slug}#${slug}`}
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="mt-1 block text-xs text-[var(--dd-subtle)]">{getCategoryCopy(locale, entry.category).label}</span>
