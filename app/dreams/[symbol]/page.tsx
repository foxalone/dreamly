...
function InterpretationSection({
  ...
  return (
    <section id={id} className="scroll-mt-24 border-t border-white/8 py-10 sm:py-14">
      <div className="grid gap-6 md:grid-cols-[190px_1fr] md:gap-12">
        <div>
          ...
          <p className="mt-4 text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">{eyebrow}</p>
          <h2 className="mt-1.5 text-xl font-semibold tracking-tight text-white">{title}</h2>
        </div>
        <div className="space-y-5 text-[15px] leading-7 text-zinc-300 sm:text-base sm:leading-8">
  ...
  return (
    <main>
      ...
      <article className="mx-auto max-w-5xl px-5 pb-12 pt-8 sm:px-8 sm:pt-12">
        <Link href="/dreams" className="inline-flex items-center gap-2 text-sm text-zinc-500 transition hover:text-white">
          <ArrowLeft size={15} aria-hidden="true" /> All dream symbols
        </Link>

        <header className="relative mt-8 overflow-hidden rounded-[2rem] border border-white/8 bg-[#111117] px-6 py-10 sm:px-10 sm:py-14">
          <div
          ...
          <div className="relative grid items-center gap-8 md:grid-cols-[1fr_190px]">
            <div>
              ...
              <p className="mt-5 max-w-2xl text-base leading-7 text-zinc-400 sm:text-lg sm:leading-8">{entry.shortMeaning}</p>
            </div>
        ...
        <section id="meaning" className="scroll-mt-24 py-12 sm:py-16">
          <div className="grid gap-6 md:grid-cols-[190px_1fr] md:gap-12">
            <div>
              ...
              <p className="mt-4 text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">Core symbol</p>
            ...
            <div>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                {entry.scenarios.map((scenario) => (
                  <div key={scenario.title} className="rounded-2xl border border-white/8 bg-white/[0.03] p-4">
                    <h3 className="text-sm font-semibold text-zinc-100">{scenario.title}</h3>
                    <p className="mt-2 text-sm leading-6 text-zinc-500">{scenario.meaning}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <section id="questions" className="scroll-mt-24 border-t border-white/8 py-10 sm:py-14">
          <div className="grid gap-6 md:grid-cols-[190px_1fr] md:gap-12">
            <div>
              <span className="grid size-11 place-items-center rounded-2xl" style={{ backgroundColor: `${entry.accent}18`, color: entry.accent }}>
                <CircleHelp size={21} strokeWidth={1.8} aria-hidden="true" />
              </span>
              <p className="mt-4 text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">Popular questions</p>
              <h2 className="mt-1.5 text-xl font-semibold tracking-tight">People also ask</h2>
            </div>
            <div className="space-y-3">
              {entry.questions.map(({ question, answer }) => (
                <details key={question} className="group rounded-2xl border border-white/8 bg-white/[0.03] p-5 open:bg-white/[0.05]">
                  <summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-sm font-semibold text-zinc-100 sm:text-base">
                    {question}
                    <span className="text-xl font-light text-zinc-500 transition group-open:rotate-45">+</span>
                  </summary>
                  <p className="mt-4 pr-6 text-sm leading-7 text-zinc-400">{answer}</p>
                </details>
              ))}
            </div>
          </div>
        ...
        <section id="related" className="scroll-mt-24 border-t border-white/8 py-10 sm:py-14">
          <div className="flex items-end justify-between gap-5">
            <div>
              <span className="grid size-11 place-items-center rounded-2xl" style={{ backgroundColor: `${entry.accent}18`, color: entry.accent }}>
                <Compass size={21} strokeWidth={1.8} aria-hidden="true" />
              </span>
              <p className="mt-4 text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">Keep exploring</p>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            {entry.related.map(({ slug, reason }) => {
              ...
              return (
                <Link key={slug} href={`/dreams/${slug}`} className="group rounded-2xl border border-white/8 bg-white/[0.03] p-5 transition hover:-translate-y-0.5 hover:border-white/20 hover:bg-white/[0.06]">
                  <div className="flex items-center justify-between">
                    <span className="text-3xl" aria-hidden="true">{related.emoji}</span>
                    <ArrowRight size={16} className="text-zinc-600 transition group-hover:translate-x-1 group-hover:text-white" aria-hidden="true" />
                  </div>
                  <h3 className="mt-4 font-semibold text-zinc-100">{related.symbol} dreams</h3>
        ...
        <aside className="rounded-2xl border border-amber-300/12 bg-amber-300/[0.045] p-5 text-xs leading-6 text-zinc-500">
