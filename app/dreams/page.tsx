...
export default function DreamDictionaryPage() {
  ...
  return (
    <main>
      ...
      <section className="relative overflow-hidden border-b border-white/8 px-5 pb-14 pt-16 sm:px-8 sm:pb-20 sm:pt-24">
        ...
        <div className="relative mx-auto max-w-4xl text-center">
          <div className="mx-auto mb-6 flex w-fit items-center gap-2 rounded-full border border-violet-400/20 bg-violet-400/10 px-4 py-2 text-xs font-medium text-violet-200">
          ...
          <div className="mx-auto mt-8 flex max-w-lg items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.045] px-4 py-3 text-left text-sm text-zinc-500">
            <Search size={17} className="shrink-0 text-violet-300" aria-hidden="true" />
      ...
      <section className="mx-auto max-w-6xl px-5 py-10 sm:px-8 sm:py-14" aria-labelledby="symbol-nav-title">
        <h2 id="symbol-nav-title" className="text-center text-xs font-semibold uppercase tracking-[0.22em] text-zinc-500">
          Jump to a symbol
        </h2>
          ...
          {DREAM_SLUGS.map((slug) => {
            ...
            return (
              <a
                key={slug}
                href={`#${slug}`}
                className="group flex flex-col items-center gap-2 rounded-2xl border border-white/8 bg-white/[0.035] px-2 py-4 transition hover:-translate-y-1 hover:border-white/20 hover:bg-white/[0.07]"
              >
                <span className="text-3xl transition group-hover:scale-110" aria-hidden="true">{entry.emoji}</span>
                <span className="text-xs font-medium text-zinc-400 group-hover:text-white">{entry.symbol}</span>
              </a>
            );
          })}
        </nav>
          ...
          {DREAM_SLUGS.map((slug, index) => {
            ...
            return (
              <article
                key={slug}
                id={slug}
                className="scroll-mt-28 overflow-hidden rounded-3xl border border-white/8 bg-[#111117]"
              >
                <div className="grid gap-0 md:grid-cols-[220px_1fr]">
                  <div
                    className="flex min-h-40 items-center justify-center border-b border-white/8 md:min-h-56 md:border-b-0 md:border-r"
                  ...
                  <div className="p-6 sm:p-8">
                    <div className="flex items-start justify-between gap-5">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">Symbol {String(index + 1).padStart(2, "0")}</p>
                    ...
                    <p className="mt-4 max-w-2xl leading-7 text-zinc-400">{entry.shortMeaning}</p>
                    <div className="mt-6 flex flex-wrap gap-2">
                      {entry.scenarios.slice(0, 3).map((scenario) => (
                        <span key={scenario.title} className="rounded-full border border-white/8 bg-white/[0.035] px-3 py-1.5 text-xs text-zinc-400">
                          {scenario.title}
                        </span>
                      ))}
                    </div>
        ...
        <aside className="mt-12 rounded-3xl border border-amber-300/12 bg-amber-300/[0.045] p-6 text-sm leading-6 text-zinc-400 sm:p-8">
          <p className="font-semibold text-zinc-200">A note about dream meanings</p>
