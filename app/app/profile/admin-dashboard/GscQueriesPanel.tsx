...
export default function GscQueriesPanel() {
  ...
  const load = useCallback(async (nextSort: SortKey = sort) => {
    setLoading(true);
    setError(null);
    try {
      const [statusPayload, queriesPayload] = await Promise.all([
        adminFetch("/api/admin/gsc/status"),
        adminFetch(`/api/admin/gsc/queries?limit=250&sort=${nextSort}`),
      ]);
  ...
  const rangeLabel = useMemo(() => {
    if (!status?.lastStartDate || !status?.lastEndDate) return "последние 90 дней";
    return `${status.lastStartDate} → ${status.lastEndDate}`;
  }, [status]);
    ...
    <section className="mt-6 space-y-4">
      <div className="rounded-3xl border border-[var(--border)] bg-[color-mix(in_srgb,var(--card)_70%,transparent)] p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--muted)]">
              Клики и показы по запросам, как в Performance. Снимок за {rangeLabel}.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <button
              type="button"
              onClick={() => void load(sort)}
        ...
        <div className="mt-4 grid gap-3 text-sm text-[var(--muted)] sm:grid-cols-2">
          ...
          <div>
            ...
            {typeof status?.lastRowCount === "number" ? ` · ${status.lastRowCount} запросов` : ""}
          </div>
          <div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-xs text-[var(--muted)]">
          {rows.length} запросов · это Google Search, не поиск внутри сайта
        </p>
            ...
            {rows.length === 0 && !loading ? (
              <tr>
                <td colSpan={5} className="p-4 text-[var(--muted)]">
                  Пока нет снимка. После настройки Google нажмите «Синхронизировать сейчас».
                </td>
              </tr>
            ) : (
              rows.map((row) => (
