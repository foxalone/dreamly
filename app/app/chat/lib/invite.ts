...
export function getChatInviteUrl(): string {
  return INVITE_URL;
}
...
export function getWhatsAppShareUrl(inviteUrl: string, text = DEFAULT_INVITE_TEXT): string {
  return `https://wa.me/?text=${encode(`${text} ${inviteUrl}`)}`;
}

export function getTelegramShareUrl(inviteUrl: string, text = DEFAULT_INVITE_TEXT): string {
  return `https://t.me/share/url?url=${encode(inviteUrl)}&text=${encode(text)}`;
}

export function getSmsShareUrl(inviteUrl: string, text = DEFAULT_INVITE_TEXT): string {
